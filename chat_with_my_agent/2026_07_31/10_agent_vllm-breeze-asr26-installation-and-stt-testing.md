...
- Breeze-ASR-26 針對閩南語/中文最佳化，適合臺語場合
...
**引數設定**：
- `TP_SIZE=1`：單卡執行（有 126GB VRAM 但其他服務也用 GPU）
- `GPU_MEM_UTIL=0.08`：~9.58 GB（2.88 GB 權重 + 5.62 GB KV cache + 其他）
- `--async-scheduling`：非同步排程，提升吞吐量
...
| meeting_original.wav | 30s (0-30s) | 有結果 | 會議討論內容 |
| meeting_30_60s.wav | 30s (30-60s) | 有結果 | 提到客戶劉邦安、保養 |
...
| vllm serve 被 kill | `pkill -f "vllm serve"` 太寬泛 | 改用 `kill <PID>` 針對特定程式 |
...
- [vLLM ASR 檔案](https://docs.vllm.ai/en/latest/serving/online_serving/speech_to_text.html)
