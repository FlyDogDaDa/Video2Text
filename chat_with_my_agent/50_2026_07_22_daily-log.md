...
- `tests/test_e2e_separation.py`：新增 E2E 測試腳本
...
| 問題 | 解決方案 |
|------|----------|
| `serve/sam-audio/api/service.py` 直接 import `modules.sam_audio`（循環依賴） | 改為呼叫獨立的 `serve/sam-audio/sam_audio_core.py` |
...
- [架構設計文件](chat_with_my_agent/49_2026_07_22_architecture-restructure.md)
