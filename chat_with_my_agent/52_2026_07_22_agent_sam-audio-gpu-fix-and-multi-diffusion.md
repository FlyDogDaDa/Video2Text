...
伺服器雖然偵測到 GPU 可用，但音訊分離仍跑在 CPU 上，導致處理時間極長（超過 10 分鐘）。原因是 `malloc_fraction` 參數格式錯誤，導致 CUDA 初始化失敗，模型最終 fallback 到 CPU 執行。
...
根據 [研究員回應](https://github.com/facebookresearch/sam-audio/issues/49)，模型訓練時使用的音訊片段通常只有 **10 秒**。120 秒音訊的 OOM 問題，根源在於 DAC-VAE 編碼率為 25 Hz，120 秒音訊會產生 3000 tokens，導致自注意力矩陣為 9M 元素，造成 GPU OOM。
...
def separate(
    ...
    device: str = "cpu",
    window_seconds: float = 30.0,   # 新增參數
    overlap_seconds: float = 3.0,    # 新增參數
) -> tuple[Path, Path]:
