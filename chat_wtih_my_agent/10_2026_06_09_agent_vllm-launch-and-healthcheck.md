...
# vLLM 啟動腳本與 API 健康檢查工具

## What

建立兩個 shell 腳本，簡化 Gemma-4-12B vLLM 服務啟動與 API 驗證流程：
...
- `try_video.py` 的 LLM 定義包含多個關鍵參數：`compressed-tensors`、`enforce-eager`、`256K max_model_len`、TRITON_ATTN 等
- 每次手動鍵入長命令容易遺漏或打錯，需要一個可覆用的啟動腳本
- 從 `.env` 讀取 `HF_TOKEN` 避免重複輸入認證

### Healthcheck Script

- vLLM 啟動需要數十分鐘，無法預知何時就緒
- 在發送 API 呼叫前，自動等待「Application startup complete.」或偵測到 error
...
**根因：** 啟動腳本第 25 行縮排錯誤（1 個空格 + 末尾 `\`），讓 `--enforce-eager` 被 vLLM CLI 當成 **positional argument**（model name），而非 flag。
