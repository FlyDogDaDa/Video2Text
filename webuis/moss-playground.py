...
頁面流程（由上而下）：
    ...
    4. 處理 → POST /v1/audio/transcriptions（vLLM、json、串行佇列）
    5. 輸出（原文）：模型原始簡體文字（含 [start][Sxx]…[end] 標記）
    6. 最終輸出（僅在有原文輸出時顯示）：OpenCC s2t 繁體＋regex 解析的 segments JSON
    7. 狀態資訊（最末行）
...
def make_ui() -> gr.Blocks:
    with gr.Blocks(title="MOSS-TD Playground") as ui:
        gr.Markdown(
            ...
            "單頁體驗：上傳音訊 → 處理 → 輸出（原文）→ 最終輸出（繁體＋JSON）\n\n"
        ...
        run_btn = gr.Button("🚀 處理", variant="primary", size="lg")
        ...
        def on_run(audio, hotwords):
            ...
            try:
                ...
                yield "", gr.update(visible=False), "", "", f"❌ 處理失敗：{e}"

        def on_clear_all():
            return None, "", "", gr.update(visible=False), "", "", "✅ 已清空"

        run_btn.click(
            fn=on_run,
            inputs=[audio_input, hotwords_input],
            outputs=[raw_output, final_group, tw_output, json_output, status_md],
        )
        clear_all_btn.click(
            fn=on_clear_all,
            inputs=None,
